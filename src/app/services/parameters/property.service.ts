...
export class PropertyService {
  ...
  getAllProperties(pag: number): Observable<any> {
    ...
    return this.http.get<boolean>(
      `${this.urlLogic}/public-property?filter={"skip":${skip},"limit":${limit}}`
    );
  }
  ...
  getProperties(filter: any): Observable<any> {
    ...
    return this.http.get<boolean>(
      `${this.urlLogic}/public-property?filter=${queryParams.toString()}`
    );
  }
  ...
  getPhotos(filter: any): Observable<any> {
    ...
    return this.http.get<boolean>(
      `${this.urlLogic}/photo?filter=${queryParams}`
    );
  }
  ...
  uploadPhoto(photo: FormData): Observable<any> {
    return this.http.post<boolean>(
      `${this.urlLogic}/upload-property-file`,
      photo
    );
  }
